...
def _prim_regresion_lineal(X, y):
    """
    Ajusta regresión lineal por mínimos cuadrados ordinarios.
    Resuelve β = (X^T X)^{-1} X^T y usando eliminación de Gauss-Jordan.
    X: lista de listas [[x1], [x2], ...] o [[1, x1], [1, x2], ...] con columna de unos.
    y: lista de valores objetivo [y1, y2, ...].
    Retorna lista de coeficientes [β0, β1, ...] o None si X^T X es singular.
    """
    n = len(X)
    p = len(X[0])
    # Construir X^T X (p x p)
    XtX = [[0.0] * p for _ in range(p)]
    for i in range(p):
        for j in range(p):
            for k in range(n):
                XtX[i][j] += X[k][i] * X[k][j]
    # Construir X^T y (p x 1)
    Xty = [0.0] * p
    for i in range(p):
        for k in range(n):
            Xty[i] += X[k][i] * y[k]
    # Resolver (X^T X) β = X^T y con Gauss-Jordan
    # Matriz aumentada [XtX | Xty]
    M = [XtX[i][:] + [Xty[i]] for i in range(p)]
    for col in range(p):
        # Pivoteo parcial
        max_fila = max(range(col, p), key=lambda r: abs(M[r][col]))
...
def _prim_r2_score(y_real, y_pred):
    """
    Coeficiente de determinación R².
    R² = 1 - SS_res / SS_tot
    Retorna float en (-inf, 1]. Más cercano a 1 = mejor ajuste.
    """
    n = len(y_real)
    media = sum(y_real) / n
    ss_tot = sum((y_real[i] - media) ** 2 for i in range(n))
    ss_res = sum((y_real[i] - y_pred[i]) ** 2 for i in range(n))
    if abs(ss_tot) < 1e-15:
        return 1.0 if abs(ss_res) < 1e-15 else 0.0
...
class VisitanteEvaluador(RISCOVisitor):
    ...
    def _builtin_ml_ajustar_lineal(self, args):
        """
        prim_ml_ajustar_lineal(X, y) → Result<[Decimal]>
        X: matriz con columna de 1s ya añadida [[1, x1], ...]
        y: vector objetivo
        Devuelve Ok(coeficientes) o Err(msg).
        """
        if len(args) != 2:
            raise Exception(f"prim_ml_ajustar_lineal() requiere 2 argumentos")
        X, y = args
        if not isinstance(X, list) or not isinstance(y, list):
            raise Exception("prim_ml_ajustar_lineal() requiere una matriz y un vector")
        self.gestor_memoria.validar_matriz(X, "X")
        resultado = _prim_regresion_lineal(X, y)
        if resultado is None:
            return ("err", "ml_ajustar: matriz X^T X singular, verifica los datos")
        return ("ok", resultado)
    ...
    """
        if len(args) != 2:
            raise Exception(f"prim_ml_r2() requiere 2 argumentos")
        y_real, y_pred = args
        if not isinstance(y_real, list) or not isinstance(y_pred, list):
            raise Exception("prim_ml_r2() requiere dos vectores")
